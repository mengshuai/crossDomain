(function() {
    ...
    */
    var root = this;
    if(window.__isLoadClient){
        throw new Error("only one instance of iTalentSDKSClient is allowed!");
    }
    window.__isLoadClient = true;
    ...
    var iTalentSDK = {
                ...
                if(scrollY || scrollX){
                    if(window.ET && BSGlobal.env == 'Production'){
                        window.ET.getExtendEtParams = function() {
                            return {
                                'userId': BSGlobal.loginUserInfo.Id, // 各产品线根据自己的业务返回 userId 和 tenantId
                                'tenantId': BSGlobal.tenantInfo.Id,
                                'applicationName': BSGlobal.appName || "sdk", // applicantionName 必要
                            };
                        }
        ...
        ,_updateSize : function(data){
            ...
            if(data['height']+"px" != iframeItem.height){
        ...
        ,showPopup : function(url, width, height, data,transferFrameUrl,childPosition,isNeedAnimate,isNeedSetWidthAndHeight) {
            var self = this;
            window.top.postMessage(this._convertMessageParams({
                ...
                //_childIsNeedBorderAndShadow : childIsNeedBorderAndShadow == false ? false : true,
                _iTalentType : 'showPopup',
                publishData : self._convertMessageParams(data)
            }), "*")
        }
        ,hidePopup : function(isNeedAnimate) {
        ...
        ,_setPopIframePositon : function(node, width, height,isNeedAnimate) {
            isNeedAnimate = isNeedAnimate == false  ? false : true;
            width = width && this._convertMetricToPX(width, true);
            height = height && this._convertMetricToPX(height, true);
            var ifOldIE = this._checkBrowserVersion();
            if(ifOldIE || !isNeedAnimate){
